...
var corsOptions = {
  origin: [
    "http://localhost:3000",
    "https://www.merisaarinen.tech",
    "https://merisaarinen-web.azurewebsites.net",
  ],
...
app.post("/sendEmail", jsonParser, async (req, res) => {
  if (req.hostname === "localhost") res.sendStatus(200);
  try {
    ...
    transporter.sendMail(mailOptions, function (error, info) {
      if (error) {
        res
          .status(400)
          .json({ status: 400, message: `Something went wrong. ${err}` });
      } else {
